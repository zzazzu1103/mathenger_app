...
def import_pair(
    ...
    """HWP(필수) + 엑셀(선택)을 임포트한다. 같은 파일은 다시 넣지 않는다."""
    report = ImportReport(source_name=hwp_name)

    existing = db.find_source_by_hash(conn, hwp_bytes)
    if existing:
        raise ValueError(
            f"같은 내용의 파일이 이미 '{existing['name']}' (원본 #{existing['id']})으로 등록되어 있습니다."
        )
    ...
    source_id = db.add_source(conn, hwp_name, hwp_bytes, len(result.problems))
